...
/**
 * Heartbeat отправляется именованным событием, а не комментарием.
 *
 * Комментарий решал бы только половину задачи — не давал прокси разорвать
 * простаивающее соединение. Но комментарии не доходят до JS, поэтому клиент не мог
 * бы отличить живой поток от полуоткрытого: на телефоне соединение часто умирает
 * молча, и человек сидел бы с замершим табло, считая, что всё хорошо.
 * Событие без `id` виден клиенту и не сдвигает Last-Event-ID.
 */
export const HEARTBEAT = 'event: ping\ndata: {}\n\n'
